  ...
  try {
    ...
    if (!sk || !pk) {
      return res.status(400).json({
        error:
          "Chaves ausentes: configure PIXONE_SK/PIXONE_PK nas variáveis do Vercel ou envie em auth.",
      });
    }
    ...
    // Se PixOne devolveu erro, repassa
    if (!upstream.ok) {
    ...
    return res.status(200).json({
      ...
      raw: data, // debug (pode remover depois)
    });
  } catch (e) {
